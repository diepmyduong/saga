...
import {
  ...
  Filter,
  MetaKey,
} from "../resource-discovery/resource-discovery.interface";
...
export const withMetaAtKey: (key: MetaKey) => Filter<DiscoveredClass> = (key) => (component) => {
  ...
  const metaTargets: (Function | undefined)[] = [
    get(component, "instance.constructor"),
    // eslint-disable-next-line @typescript-eslint/ban-types
    component.injectType as Function,
  ].filter((x) => x != null);

  return some(metaTargets, (x) => Reflect.getMetadata(key, x as any));
};
...
export class DiscoveryService {
  ...
  async methodsAndControllerMethodsWithMetaAtKey<T>(
    ...
    const controllersWithMeta = (await this.controllersWithMetaAtKey<T>(metaKey)).filter((x) => metaFilter(x.meta));

    const methodsFromDecoratedControllers = flatMap(controllersWithMeta, (controller) => {
      return this.classMethodsWithMetaAtKey<T>(controller.discoveredClass, PATH_METADATA);
    });

    const decoratedMethods = (await this.controllerMethodsWithMetaAtKey<T>(metaKey)).filter((x) => metaFilter(x.meta));

    return uniqBy([...methodsFromDecoratedControllers, ...decoratedMethods], (x) => x.discoveredMethod.handler);
  }
