...
import { cloneDeep, forEach, get, isBoolean, isEmpty, isNumber, isObject, isString, startsWith } from "lodash";
...
export class WorkSheetHelper {
  static responseExcel(res: Response, workBook: Workbook, filename = "baocao") {
    ...
    res.setHeader("Content-disposition", `attachment; filename=${filename.replace(/\ /g, "-")}.xlsx`);
  ...
  public copyCellRange(rangeSrc: CellRange, rangeDest: CellRange, sheetSrc?: Worksheet, sheetDest?: Worksheet): void {
  ...
  private getMergeRange(cell: Cell, sheet?: Worksheet) {
    ...
    if (cell.isMerged && Array.isArray((sheet as any).model["merges"])) {
      ...
      const cellRangeStr = (sheet as any).model["merges"].find((item: string) => item.startsWith(address + ":"));
      if (cellRangeStr) {
  ...
  private copyCell(cellSrc: Cell, cellDest: Cell, sheetSrc?: Worksheet, sheetDest?: Worksheet): void {
    if (!sheetSrc) sheetSrc = this.worksheet;
    if (!sheetDest) sheetDest = this.worksheet;
...
export function replaceDataToText(text: string, data: any) {
  ...
  return messageText.replace(/\:\"(?: +)?<<Object\((true|false|[\{|\[].*?[\}|\]])\)Object>>(?: +)?\"/g, ":$1");
}
